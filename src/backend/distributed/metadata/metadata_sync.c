...
NodeListInsertCommand(List *workerNodeList)
{
	...
	foreach_declared_ptr(workerNode, workerNodeList)
	{
		...
		StringInfo nodeprimarynodeidString = makeStringInfo();

		if (workerNode->nodeprimarynodeid == 0) /* Assuming 0 is InvalidNodeId */
		{
			appendStringInfoString(nodeprimarynodeidString, "NULL");
		}
		else
		{
			appendStringInfo(nodeprimarynodeidString, "%d", workerNode->nodeprimarynodeid);
		}
		...
		appendStringInfo(nodeListInsertCommand,
						 "(%d, %d, %s, %d, %s, %s, %s, %s, '%s'::noderole, %s, %s, %s, %s)",
						 workerNode->nodeId,
						 workerNode->groupId,
