...
CREATE SCHEMA background_rebalance_parallel_fk;
SET search_path TO background_rebalance_parallel_fk;

-- Create two reference tables with a foreign key relationship
CREATE TABLE referenced_table (id int PRIMARY KEY);
SELECT create_reference_table('referenced_table');

CREATE TABLE referencing_table (
    id int PRIMARY KEY,
    ref_id int REFERENCES referenced_table(id)
);
...
-- Start the rebalance, which will trigger the copy of reference tables
